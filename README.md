...
Once the form has been created, use the <code>setFormAttr</code> function to change the default attributes:

```php
// Add a new form action
$new_form->setFormAttr('action', 'http://submit-here.com');

// Change the submit method
$new_form->setFormAttr('method', 'get');

// Change the enctype
$new_form->setFormAttr('enctype', 'multipart/form-data');

// Can be set to 'html' or 'xhtml'
$new_form->setFormAttr('markup', 'xhtml');

// Classes are added as an array
$new_form->setFormAttr('class', array());

// Add an id to the form
$new_form->setFormAttr('id', 'xhtml');

// Adds the HTML5 "novalidate" attribute
$new_form->setFormAttr('novalidate', true);

// Adds a WordPress nonce field using the string being passed
$new_form->setFormAttr('add_nonce', 'build_a_nonce_using_this');

// Adds a blank, hidden text field for spam control
$new_form->setFormAttr('add_honeypot', true);

// Wraps the inputs with a form element
$new_form->setFormAttr('form_element', true);
...
```php
$new_form->add('I am a little field', array(), 'little_field')
```
